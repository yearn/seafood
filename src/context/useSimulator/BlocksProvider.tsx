...
export default function BlocksProvider({
	...
	const findHarvest = useCallback((blocks: Block[], strategy: Strategy) => {
	...
	const findDebtRatioUpdate = useCallback((blocks: Block[], vault: Vault, strategy: Strategy) => {
		return blocks.findIndex(b =>
	...
	const addHarvest = useCallback(async (vault: Vault, strategy: Strategy) => {
		...
		setBlocks(current => {
			const index = findHarvest(current, strategy);
			if(index > -1) return current;
			return [...current, block];
		});
	}, [setBlocks, provider, findHarvest]);
	...
	const addDebtRatioUpdate = useCallback(async (vault: Vault, strategy: Strategy, debtRatio: number) => {
		...
		setBlocks(current => {
			const result = [...current];
			const debtRatioUpdateIndex = findDebtRatioUpdate(result, vault, strategy);
			if(debtRatioUpdateIndex > -1) result.splice(debtRatioUpdateIndex, 1);
			const harvestIndex = findHarvest(result, strategy);
	...
	}, [setBlocks, provider, findDebtRatioUpdate, findHarvest]);
	...
	}, [setBlocks, findDebtRatioUpdate, findHarvest]);
