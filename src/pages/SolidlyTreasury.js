...
function SolidlyTreasury(){
	...
	useEffect(() => {
		getAll(providerByChainId(chainId('fantom')));
	}, [providerByChainId]);
		...
		return <div>
			...
			{lps.map((lp) => (
				<div key={lp.address}> <h3>{lp.name}</h3>
					...
					<ul>
						<li><A target={'_blank'} rel={'noreferrer'} href={getAddressExplorer(chainId('fantom'), lp.address)}> {'lp: ' + lp.address} </A></li>
						<li><A target={'_blank'} rel={'noreferrer'} href={getAddressExplorer(chainId('fantom'), lp.tokenABalance.address)}> {FindName(lp.tokenABalance.address) + ' balance: ' + formatNumber(lp.tokenABalance.balance)} </A></li>
						<li><A target={'_blank'} rel={'noreferrer'} href={getAddressExplorer(chainId('fantom'), lp.tokenBBalance.address)}> {FindName(lp.tokenBBalance.address) + ' balance: ' + formatNumber(lp.tokenBBalance.balance)} </A></li>
						<li><A target={'_blank'} rel={'noreferrer'} href={getAddressExplorer(chainId('fantom'), sex())}> {'sex pending rewards: '  + formatNumber(lp.sexRewards) + ' worth $' + formatNumber(lp.sexRewards*prices[sex()])} </A></li>
						<li><A target={'_blank'} rel={'noreferrer'} href={getAddressExplorer(chainId('fantom'), solid())}> {'solid pending rewards: '  + formatNumber(lp.solidRewards) + ' worth $' + formatNumber(lp.solidRewards*prices[solid()])} </A></li>
			...
			<div>
				{solidsexStaked && <ul>
					<li><A target={'_blank'} rel={'noreferrer'} href={getAddressExplorer(chainId('fantom'), solidsex())}> {'staked solidex: '  + formatNumber(solidsexStaked.tokenABalance.balance)} </A></li>
					<li><A target={'_blank'} rel={'noreferrer'} href={getAddressExplorer(chainId('fantom'), sex())}> {'sex pending rewards: '  + formatNumber(solidsexStaked.sexRewards) + ' worth $' + formatNumber(solidsexStaked.sexRewards*prices[sex()])} </A></li>
					<li><A target={'_blank'} rel={'noreferrer'} href={getAddressExplorer(chainId('fantom'), solid())}> {'solid pending rewards: '  + formatNumber(solidsexStaked.solidRewards) + ' worth $' + formatNumber(solidsexStaked.solidRewards*prices[solid()])} </A></li>
				</ul>}
			</div>
			<h2>{'Staked Sex:'}</h2>
			<div>
				{sexStaked && <ul>
					...
					<li><A target={'_blank'} rel={'noreferrer'} href={getAddressExplorer(chainId('fantom'), solidsex())}> {'solidsex pending rewards: '  + formatNumber(sexStaked.solidsexRewards) + ' worth $' + formatNumber(sexStaked.solidsexRewards*prices[solidsex()])} </A></li>
				</ul>}
			</div>
			<h2>{'Staked Nft:'}</h2>
			<div>
				{solidNft && <ul>
					...
					<li><A target={'_blank'} rel={'noreferrer'} href={getAddressExplorer(chainId('fantom'), solid())}> {'staked solid Nft: '  + formatNumber(solidNft.tokenABalance.balance) + ' worth $' + formatNumber(solidNft.tokenABalance.balance*prices[solid()])} </A></li>
				</ul>}
			</div>
		</div>;
	}else{
